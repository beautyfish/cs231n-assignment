...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    dists = np.sqrt( - 2 * M + te.reshape(-1, 1) + tr)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      max_num = 0
      m_label = 0
      for lab in set(closest_y):
        if closest_y.count(lab) > max_num:
            m_label = lab 
            max_num = closest_y.count(lab)
        elif closest_y.count(lab) == max_num:
            m_label = min(m_label, lab)
      y_pred[i] = m_label
      #########################################################################
      #                           END OF YOUR CODE                            # 
      #########################################################################
